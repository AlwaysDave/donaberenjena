...
async function startServer() {
  ...
  app.post("/api/parse-cata", upload.single("file"), async (req, res) => {
    try {
      ...
      const prompt = `You are an expert sommelier and data extraction assistant. 
Analyze the provided promotional poster or document for a wine/vermouth tasting (cata).
Extract the details into the provided JSON schema. 
Pay special attention to whether it's a single winery (bodega_unica) or multiple wineries/passes (varias_bodegas).
