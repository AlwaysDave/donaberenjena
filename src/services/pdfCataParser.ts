...
export function parseCataText(rawText: string): ParsedCataInfo {
  ...
  let cataType: 'bodega_unica' | 'varias_bodegas' = 'bodega_unica';
  let tallerEspecial = '';

  // Clean whole text for regex matching if needed
  const fullText = lines.join('\n');

  // Detect multi-bodega tasting keywords ("Pase I", "Pase II", or multiple distinct bodegas)
  const hasMultiplePases = /pase\s+i|pase\s+ii|pase\s+iii/i.test(fullText);
  const bodegaMatchesCount = (fullText.match(/bodegas?\s+/gi) || []).length;
  if (hasMultiplePases || bodegaMatchesCount >= 2) {
    cataType = 'varias_bodegas';
  }

  // Taller especial detection e.g. "VAS A HACER TU PROPIO VERMUT" or "DURANTE LA CATA LOS ASISTENTES PODRÁN ELABORAR"
  for (const line of lines) {
    if (/vas\s+a\s+hacer\s+tu\s+propio/i.test(line) || /durante\s+la\s+cata\s+los\s+asistentes/i.test(line) || /elaborar\s+su\s+propio/i.test(line)) {
      tallerEspecial = line;
      break;
    }
  }
  ...
  return {
    ...
    cataType: cataType || 'bodega_unica',
    tallerEspecial: tallerEspecial || '',
